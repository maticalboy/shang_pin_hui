    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImgList="skuImageList"/>
          <!-- 小图列表 -->
          <ImageList :skuImgList="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="p in spuSaleAttrList" :key="p.id">
                <dt class="title">选择{{p.saleAttrName}}</dt>
                <dd @click="changeActive(pp,p.spuSaleAttrValueList)" :class="{active:pp.isChecked=='1'}" changepirce="0" v-for="pp in p.spuSaleAttrValueList" :key="pp.id">{{pp.saleAttrValueName}}</dd>
              </dl>
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a @click="add" href="javascript:" class="plus">+</a>
                <a @click="sub" href="javascript:" class="mins">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addOrUpdateShopCart(skuInfo.id,skuNum)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods:{
    changeActive(selectValue,attr){
      attr.forEach(el => {
        el.isChecked="0"
      });
      selectValue.isChecked="1"
    },
    add(){
      this.skuNum++
    },
    ...
    async addOrUpdateShopCart(skuId,skuNum){
      try {
        ...
        this.$router.push({
          path:'/addcartsuccess',
          query:{
            // skuInfo:this.skuInfo,//传对象路径不好看 用session
            skuNum:skuNum
          }
        })
      } catch (error) {
