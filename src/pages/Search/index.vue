            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active:isOne  }" @click="changeOrder('1')">
                  <a href="#">综合<span v-show="isOne" class="iconfont" :class="{'icon-arrowup':isUp,'icon-arrowdown':isDown}"></span></a>
                  
                </li>
                <li :class="{ active:isTwo  }" @click="changeOrder('2')">
                  <a href="#">价格<span v-show="isTwo" class="iconfont" :class="{'icon-arrowup':isUp,'icon-arrowdown':isDown}"></span></a>
            ...
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="(p) in goodsList" :key="p.id">
                <div class="list-wrap">
                  ...
                  <div class="attr">
                    <a target="_blank" href="item.html" :title="p.title">{{ p.title }}</a>
                  </div>
                  <div class="commit">
...
export default {
  ...
  methods: {
    ...
    attrInfo(id, value, name,) {
      ...
      if (this.searchParams.props.indexOf(`${id}:${value}:${name}`) == -1) {
        this.searchParams.props.push(`${id}:${value}:${name}`)
        this.getSearchData()
      }
    ...
    changeOrder(flag){
      ...
      if(flag==originFlag){
        // 上一次也是综合
        if(originSort=="desc"){
          this.searchParams.order=`${originFlag}:"asc"`
        }else{
          this.searchParams.order=`${originFlag}:desc`
        }
  ...
  watch: {
    $route: {
      ...
      handler(newValue, oldValue) {
        ...
        // console.log(newValue,oldValue,"666")
      }
    },
  }
